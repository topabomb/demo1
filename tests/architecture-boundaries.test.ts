...
describe('engine architecture boundaries', () => {
  ...
  it('keeps synthetic playback and provider policy out of engine session truth', () => {
    ...
    expect(kernel).toContain('continueExecutionAt(currentAssistantIndex: number)')
  ...
  it('keeps the public workspace scenario-focused while diagnostics remain an explicit engine surface', () => {
    ...
    expect(diagnostics).toContain('Active Agent loop')
    expect(diagnostics).toContain('data-testid="active-turn-id"')
    expect(diagnostics).toContain('data-testid="active-step-id"')
    expect(diagnostics).toContain('data-testid="active-tool-categories"')

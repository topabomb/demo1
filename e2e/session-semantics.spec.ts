...
test('Recent separates live execution, blockers and last-turn outcomes', async ({ page }) => {
  ...
  await expect(page.getByTestId('session-tool-rendering')).toContainText(/Blocked.*Approval/)
  await expect(page.getByTestId('session-android-protocol')).toContainText(/Blocked.*Question/)
...
test('approval and question are independent session-owned blockers', async ({ page }) => {
  ...
  await expect(question).toContainText('Choose target API behavior')

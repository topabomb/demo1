...
<template>
  <article class="message-card tool-card" :class="[`tool-${phase}`, `tool-${status}`]" data-testid="tool-block">
    <button class="tool-summary" type="button" @click="open = !open">
      <span class="tool-icon">{{ phase === 'result' ? '↳' : '⚙' }}</span>
      <span class="tool-title">
        <strong>{{ phase === 'result' ? 'Tool result' : 'Tool call' }} · {{ unit.payload.name }}</strong>
        <small>{{ unit.payload.callId }} · {{ duration.toLocaleString() }} ms</small>
      </span>

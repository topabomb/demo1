...
export function createMarkdownGalleryTurn(scope: string, ordinal: number): AppendCanonicalMessage[] {
  ...
  return [{
    ...
    blocks: MARKDOWN_COMPATIBILITY_FIXTURES.map(item => block(item.id, 'markdown', { markdown: item.markdown, flavor: 'gfm' })),
  }]
}
...
export function createMixedDemoTurn(scope: string, ordinal: number): AppendCanonicalMessage[] {
  ...
  const rich: ContentBlock[] = [
    block('reasoning', 'reasoning', {
      text: `I will inspect the current session state, preserve stable identities, and verify the viewport after this runtime-injected turn ${ordinal}.`,
      tokenCount: 58 + ordinal,
      durationMs: 1200 + ordinal * 73,
      defaultOpen: false,
    }),
    block('answer', 'markdown', {
      markdown: `## Runtime-injected heterogeneous turn ${ordinal}\n\nThis content was appended to the **canonical SessionKernel**, then projected through the same registry as historical and streamed content.\n\n- stable block identity\n- bounded RenderUnits\n- responsive containment\n- no DOM-side fixture shortcut`,
      flavor: 'gfm',
    }),
  ]

  if (visual === 0) rich.push(block('code', 'code', {
    language: 'typescript',
    filename: `src/runtime-fixture-${ordinal}.ts`,
    code: Array.from({ length: 46 }, (_, i) => `const fixture_${ordinal}_${i} = project(blocks[${i}], { stable: true })`).join('\n'),
  }))
  if (visual === 1) rich.push(block('diff', 'diff', {
    file: `src/runtime-${ordinal}.ts`,
    lines: Array.from({ length: 64 }, (_, i) => `${i % 3 === 0 ? '+' : i % 5 === 0 ? '-' : ' '} line ${i + 1}: renderer registry invariant ${ordinal}`),
  }))
  if (visual === 2) rich.push(block('image', 'image', {
    width: 1440,
    height: 810,
    seed: 9000 + ordinal,
    alt: `Synthetic responsive image fixture ${ordinal}`,
  }))
  if (visual === 3) rich.push(block('html', 'html', {
    html: `<section class="synthetic-html"><h3>Runtime artifact ${ordinal}</h3><p>Sanitized HTML follows the same ContentBlock contract.</p><div class="html-chip">responsive container</div><script>window.__fixtureUnsafe=true</script></section>`,
  }))
  ...
  return [
    {
      turnId,
      role: 'user',
      variant: 'fixture-user',
      blocks: [block('request', 'markdown', { markdown: `Please execute runtime fixture **${ordinal}** and demonstrate a mixed Agent turn.` })],
    },
    { turnId, role: 'assistant', variant: 'fixture-assistant', blocks: rich },
    {
      turnId,
      role: 'assistant',
      variant: 'fixture-tool-call',
      blocks: [block('tool-call', 'tool-call', {
        name: ordinal % 2 ? 'search' : 'read_file',
        callId,
        input: { path: `/workspace/fixture-${ordinal}.ts`, query: `renderer ${ordinal}`, limit: 20 },
        durationMs: 18,
        status: 'success',
        defaultOpen: false,
      })],
    },
    {
      turnId,
      role: 'tool',
      variant: 'fixture-tool-result',
      blocks: [block('tool-result', 'tool-result', {
        name: ordinal % 2 ? 'search' : 'read_file',
        callId,
        output: { rows: [{ line: 12, preview: 'stable keyed content' }, { line: 41, preview: 'responsive renderer containment' }], exitCode: 0 },
        durationMs: 32,
        status: 'success',
        defaultOpen: false,
      })],
